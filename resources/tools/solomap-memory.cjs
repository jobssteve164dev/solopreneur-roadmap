...
function printHelp() {
  console.log([
    'Usage: node resources/tools/solomap-memory.cjs retrieve [options]',
    '',
    'Options:',
...
function main() {
  ...
  if (command !== 'retrieve') throw new Error(`Unknown command: ${command}`);
  ...
  const payload = retrieveMemory(memoryRoot, projectRoot, String(args.query || ''), parseSources(args.sources), normalizeLimit(args.limit));
  if (args.json) {
