...
SoloMap 官网使用产品自有的登录、注册、邮箱验证和密码找回界面，通过 SZLK Passport Headless Auth 完成统一身份认证。普通用户不需要离开 SoloMap 官网理解或操作 Passport 的 OIDC 授权页面。

登录后的官网入口按个人项目工作台设计，当前职责是：

- 承载 SoloMap 账号状态、订阅入口和个人项目入口。
- 用真实空状态引导用户回到 VS Code 插件创建、打开和推进本地项目。
- 明确说明插件项目数据仍保存在本地工作区，不暗示已经完成云端同步。

在远程项目管理链路正式接通前，官网工作台不得伪造项目卡片、进度、统计或最近活动，也不得为了让页面显得完整而复制一套与插件平行的项目状态。
