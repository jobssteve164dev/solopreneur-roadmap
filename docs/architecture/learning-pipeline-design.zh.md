...
- `skipped`：本事件被检查过，但没有命中可复用候选条件。

这层决策用来保证学习管线不是“有候选才有痕迹”。没有候选时也要能解释为什么跳过，后续读取 summary、召回或晋升上下文时可以补齐旧事件缺失的决策记录。
...
非阻断不等于静默缺失。正常情况下，每次学习事件写入后都应同步尝试候选提炼并写入 candidate decision；如果当时失败或历史事件缺失决策，读取学习 summary、召回上下文或晋升上下文时应自动补账。
