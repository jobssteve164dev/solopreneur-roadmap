...
底层差异必须由 Harness 和安装 Agent 吸收。复杂安装、配置和修复不由插件直接执行命令；插件只发起受控 Agent 安装任务，并复验 Agent 写出的结构化结果。
...
用户在设置页看到的是每个内置增强的状态卡：

- 未安装 / 安装中 / 已安装 / 需要修复 / 检测失败。
- 已安装版本或“版本未知”。
- 安装、修复、重新检测等动作。

点击安装或修复时，SoloMap 唤起当前默认 Agent CLI，并要求 Agent 按增强安装 skill 完成下载、安装、配置、检测和结构化报告。插件不直接运行上游安装命令。

Agent 必须写入：

- `solomap.enhancement.json`
- `source.lock.json`
- `health.json`
- 安装任务指定的 `result.json`

插件复验这些文件、路径边界、health、版本和 manifest 后，才把增强写入 registry 并允许后续任务消费。复验失败的增强不得注入任务 prompt 或运行时。
...
```text
命令输出优化        已安装        版本 1.2.3        重新检测 / 修复
代码结构辅助        未安装        未安装           安装
MCP 工具描述压缩    需要修复      版本未知         修复 / 重新检测
```
...
跨 Agent Harness 负责统一项目上下文、能力候选、内置增强治理和执行闭环。
