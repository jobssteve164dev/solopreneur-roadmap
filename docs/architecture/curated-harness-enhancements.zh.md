...
第三步：插件复验 result、manifest、source lock、health 和版本后写入 registry；复验只能用更强的新证据更新已知版本和来源，不能把空版本、版本未知或简化来源写回覆盖安装 Agent 已确认的信息。安装只代表“可用”，不代表“启用”；用户必须在设置页主动启用，且可以随时禁用或卸载。
第四步：先实现只读/低副作用能力：
...
- 禁止把实验性增强安装后自动挂载到新 Agent 对话；设置页必须明确提示它们是外部增强项目，说明可能带来的收益，也说明外部项目自身可能拖慢启动、卡住命令、改写配置或产生错误输出，并提供启用、禁用和卸载动作。
- 禁止把外部增强的不稳定体验转嫁成 SoloMap 主插件体验；增强异常时必须能从设置页退回到 SoloMap 主路径。
- 禁止在 Agent 启动前自动执行重型初始化、索引构建、后台服务启动或可能长时间阻塞的增强命令；必要探测必须短超时并允许失败回退。
