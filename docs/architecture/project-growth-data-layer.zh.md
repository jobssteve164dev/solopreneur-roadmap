...
项目生长数据层聚合以下事实源；新增工作汇报的实施范围以本节契约及[生长图 UI 设计](../ui/project-growth-graph.zh.md)的 2026-09 首批范围为准。
...
- 任务登记来自项目 `.solopreneur/agent-runs/learning-tasks/`；原运行目录中的 `task-report-<sequence>.json` 是轮次报告来源。任务、运行和轮次使用已有稳定标识，查询键至少包含项目、taskId、executionLogId 和 turnId；通用 Solo 名称、模块标签和节点名称不能替代身份。
- 运行状态、原始用户目标及会话续接信息仍来自原任务与运行记录。报告 envelope 内的 outcome 和正文是执行者汇报，不取代宿主生命周期，不修改路线图事实源。
- 校验登记关系、报告所属项目与运行目录，文件来源必须在该项目允许的运行范围内；不能因报告自填路径、软链接或身份就跨项目读取或挂接。保存的原材料保留，生长层只建立可重建的查询投影。
- 原报告缺失、无效、存储失败分别保留可观察状态；合法旧记录没有新字段不等于失败。只能按文件结构和显式字段规范化，不从终端关键词补造报告。

#### 展示规范化与兼容

查询层提供任务分组、轮次列表、来源版本、内容分区、关联对象和可执行目标；前台不自行解释原始 JSON。任务摘要直接引用最近一轮的报告，标明时间与轮次，不生成新的跨轮语义总结。

| 报告字段 | 首批展示映射与兼容 |
| --- | --- |
| summary | 本轮结果，保留原意，不据其关键词推导成功 |
| unmetRequirements | 本轮声明的未完成事项；缺字段表示未知，显式空数组仅表示本轮未列事项，不证明全部历史问题经验证解决 |
| decisions / corrections | 关键做法、理由与纠偏；保留来源消息引用，不能把声明当成用户已确认决策 |
| verification | 实际检查、执行者报告结果、证据引用、版本与未验证范围；逐项与可核验证据分开 |
| commits | 仓库、完整 SHA、本任务文件范围；展示为精确提交链接和文件关联 |
| experienceUsage / lessons | 展开后查看经验采用和待复盘材料，不更改候选状态或计数 |

现有数组成员可能是字符串或不同形状的对象。字符串直接展示；对象只对明确的已知字段映射，其他可读内容保留在详情中，不出现 `[object Object]` 或把未知键当状态。未知字段不得让整份报告不可读；新增生产者字段须可选并兼容旧材料，格式错误与语义材料不足分开处理，不要求用户补表。

“产出”来自已有可定位的文件/交付引用与提交范围，不从摘要中的路径片段猜测。只有自报文件范围时标明来自汇报，回读提交 diff 后再补核验来源；不存在产物引用的纯讨论也能完整展示。

#### 模块与能力关联

关联链为：已登记任务与轮次 → 明确的运行文件/报告产物/提交文件范围 → 当前模块身份 → 已有能力关联。每条关联保留依据与来源版本；报告文件范围是声明，实际 diff 是对应版本的可核对材料，二者不覆盖彼此。

只凭项目名、标题、作者、时间窗口或相似词不能新增任务归属。共享提交必须按每个任务的明确范围挂接，范围缺失时只保留提交级关联，不把完整 diff 归给所有任务。已有能力关系若只是推断，不能据此把任务与能力的关系提升为确定事实；首批确定性筛选只使用有明确依据的关联。

源码移动、删除或模块重划分后，保留报告时的路径及提交引用，不把当前同名对象自动当成原对象。无法确认当前落点的报告仍留在项目全部汇报中，非代码任务不强制关联模块。

#### 增量读取与刷新

报告有效保存是本地索引更新信号，不依赖经验复盘完成或 Git 提交。半写入文件不消费，重复通知按轮次键及内容版本幂等更新；同轮材料变化保留可追溯的版本信息，新轮不会覆盖旧轮。重启、首次加载及漏通知时能从登记与原报告补齐查询投影。

沿既有 `project_growth.db` 查询层增加所需投影或缓存，原报告仍是来源，投影可重建；页面按任务分页、轮次按需读取，避免每次渲染递归展开全部报告或大型原始证据。报告索引不计入源码体量，不要求运行完整源码扫描才显示新报告。

“更新验证结果”是新增的显式事实刷新动作：在当前项目和所选任务范围复用 `learningReview.ts` 的提交发现、分页、完整 SHA 与检查校验，独立于语义复盘。发现和回读的证据保存到项目本地学习证据缓存，包含仓库、SHA、检查身份、观察时间、范围、完整性与缺口，供生长图和手动复盘共同读取；不能把只有扫描游标的文件当作完整证据缓存。

页面加载、普通刷新与项目切换只读本地；同范围并发远端请求合并，局部失败保留其他成功证据及旧结果，不伪装全部刷新成功。跨项目和旧请求迟到须通过项目身份、选择及请求版本检查后才更新界面，不重置筛选与展开状态。

事实刷新不调用模型、不写 candidate decisions、不生成或晋升经验，也不运行本地测试。原“运行验证分析”与“复盘经验”分别保留自己的触发和权限路径。

#### 验证时效与判断口径

每项证据至少区分来源（执行者声明、静态关联、实际执行覆盖、检查结果）、覆盖对象、版本、观察时间和可用性；证据不支持的字段保留未知。源码版本以完整提交及必要的工作区内容标识绑定，不能仅用扫描时间判断仍有效。

测试引用只能证明存在静态关系，覆盖率只能证明执行范围，检查通过只能证明该项断言；三者均不能直接证明整个用户目标完成。首批须移除仅由静态引用 100% 推导“已成形”或“全部验证”的路径，校准其派生建议与双语文案。验证后有相关变化则保留旧证据并标明当前版本待验证；没有可核对版本时不宣称最新。

新增报告不直接提升能力或模块状态；没有报告、没有 GitHub、没有源码关联不计为失败。原路线图状态单独展示，不受台账或事实刷新写入影响。
...
以下为原生长快照模型；工作汇报投影按上文新增契约扩展既有查询层，不另建用户需要维护的台账系统。
