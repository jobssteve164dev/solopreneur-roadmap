...
- 状态：设计与 Codex 首版运行时已落地；原生 TUI + Hook 支线仍受信任门禁阻塞。
- 首版目标 Agent：Codex。
- 当前运行边界：新版对话界面由设置开关显式启用，默认关闭；仅接管用户主动发起的 Codex Solo 与路线图环节对话，后台 Flow、复核、自动化和其他 Agent 保持原有非交互终端路径。
- 当前结论：Codex 首版采用 ACP 自建对话终端；原生 TUI 不作为首版主路径。Hook 生命周期信号可用，但普通本地扩展尚无“受支持、无额外警告、无需改写私有信任状态”的自动授权方式；`notify + rollout` 也无法在原生 TUI 首轮结束前可靠绑定根会话。

## 首版实现落点

- 设置中的“新版对话界面”默认关闭；旧设置记录和未携带该字段的局部更新不会误开启或重置它。
- 开启后，Codex 首轮在独立编辑器视图打开；关闭时仍先打开原有集成终端，不改变既有启动反馈。
- 独立视图支持流式回复、同会话连续输入、停止、权限确认与结构化问题；界面只展示“正在执行 / 需要你确认 / 需要你回答 / 等待你继续 / 已停止 / 连接出错”。
- 每次输入仍生成独立执行记录、状态文件和 30 秒心跳。回复结束后由既有任务哨兵处理文件变化、完成决策、对话历史、复核和通知，不建立第二套任务状态。
- 关闭独立视图不取消当前回复；活动会话保留在 Extension Host。侧边栏再次打开时优先 reveal 现有视图，Extension Host 重载后使用已保存的 Session ID 执行 `session/load`。
- 首版固定依赖经过生命周期验证的 `@agentclientprotocol/codex-acp@1.1.7`；Adapter 启动或恢复失败会把本轮结算为可见错误，不伪装为交互式成功。
