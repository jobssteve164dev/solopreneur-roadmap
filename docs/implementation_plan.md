...
本文件为专为一人公司（独立开发者/Solopreneur）设计的 VS Code 插件提供全面的技术调研、架构设计及实施蓝图。核心概念是一个通过 AI 生成的、可交互的项目路线图，路线图中的每个任务节点均可直接指派给本地安装的 AI Agent CLI 运行，且所有数据完全保存在本地并通过 Git 进行版本管理。
