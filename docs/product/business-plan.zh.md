...
目标：让用户的本地项目在明确授权下连接到协作者、可复用能力、真实反馈和商业机会，形成以项目结果为中心的独立开发者协作网络。长期边界和演进顺序见 [SoloMap 独立开发者生态与协作网络战略](./solo-developer-ecosystem-strategy.zh.md)。

交付：

- 邀请制、端到端加密的临时项目协作房间。
- 从本地项目主动生成的脱敏求助卡。
- 被采纳建议回到 Agent 下一轮或项目想法的明确动作。
- 基于真实项目结果的贡献归因。
- 精选模板、Skill、Agent 工作流和连接器能力库。
- 登录后可进入、整点 1 小时清空的短时共创大厅。
- 少量有主题、有期限、可治理的官方共创空间。
- 与主流本地 Agent 集成。
- 可选 GitHub issue 同步，但不作为主工作流。
- 条件合适时发布 Open VSX。

这一阶段不建设所有用户永久共用的公共聊天室，不做无限信息流、粉丝竞赛或泛化自由职业市场。只有协作、贡献和复用已经能够改善项目结果后，才验证付费审计、项目悬赏、能力交易和专业服务。
...
- 100+ 付费用户。
- 3-5 个可重复获客渠道。
- 邀请房间、求助卡或可复用能力能够稳定产生被项目拥有者采纳的下一步。
- 被采纳贡献可以继续追踪到完成、验证、发布或用户反馈，而不是停留在聊天或下载。
