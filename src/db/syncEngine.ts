    ...
    return [
      {
        ...
        agentCli: 'agy',
        agentPrompt: [
          '先阅读当前项目目录中的 README、docs、源码入口以及 .solopreneur/README.md（如果存在），理解这个项目当前想做什么。',
          '你的唯一主任务是直接重写 .solopreneur/roadmap.csv，不要只在终端输出路线图建议。',
          '重写时必须严格满足以下要求：',
          ...
          '3. stage 只能使用：商业规划、品牌与设置、产品与 MVP、营销与增长',
          '4. 每个环节都必须有明确的本地交付物，agentPrompt 必须要求后续 Agent 直接创建或修改项目文件，并在适用时执行最窄验证命令',
          '5. dependencies 要反映真实前置关系；第一步留空，后续环节按需要依赖前面的 id',
          '6. 每行 agentCli 都写 agy，status 全部写 Pending，completedAt 留空，createdAt 写当前 ISO 时间',
          '7. 不要生成空泛咨询任务，不要顺手改无关文件',
          '写完后重新读取 .solopreneur/roadmap.csv，自检列名、stage、环节数量和依赖关系，确认正确后再退出。'
        ].join('\n'),
        status: 'Pending',
        createdAt: now,
        completedAt: '',
