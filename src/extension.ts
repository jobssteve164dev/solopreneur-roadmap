...
function buildSoloContextIndex(workspaceRoot: string, globalDataPath: string, globalPromptPath = ''): string {
  ...
  return [
    ...
    `- 记忆写入：确认需要沉淀后，运行 \`node ${path.join(workspaceRoot, 'resources', 'tools', 'solomap-memory.cjs')} route --project ${JSON.stringify(workspaceRoot)} --global ${JSON.stringify(globalRoot)} --kind ${JSON.stringify('<profile|rules|project|decision|pattern|domain|inbox|active>')} --title ${JSON.stringify('<主题>')}\`；先读取返回的目标文件，再按更新策略、必需结构和门禁精确更新，不要盲目追加。`,
