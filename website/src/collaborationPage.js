...
export function buildCollaborationRoomPage(roomId, locale = "en") {
  const zh = locale === "zh";
  const copy = zh ? {
    title: "临时共创 · SoloMap",
    eyebrow: "临时共创房间",
    ...
    room: "临时共创",
    privacy: "消息在你的设备上加密，SoloMap 中继只保存最长 24 小时的密文。",
    nameLabel: "你的昵称",
    namePlaceholder: "输入参与者昵称",
    join: "加入共创",
    joining: "正在连接…",
    missing: "邀请链接不完整。请让房间发起者重新发送完整链接。",
    connecting: "正在连接",
    connected: "已连接",
    offline: "连接已断开",
    expired: "房间已结束",
    ...
    privacy: "Messages are encrypted on your device. The SoloMap relay keeps ciphertext for no longer than 24 hours.",
    nameLabel: "Your name",
    namePlaceholder: "Enter a participant name",
    join: "Join room",
    joining: "Connecting…",
    missing: "This invite is incomplete. Ask the room owner to send the full link again.",
    ...
    error: "The room is unavailable. Check the invite or try again shortly."
  };
...
<body>
  ...
  (() => {
    ...
    const localePrefix = ${JSON.stringify(zh ? "/zh" : "")};
    ...
    async function renderRecentRooms() { const list = document.getElementById("recent-list"); const rooms = (await readRooms()).filter(room => Number(room.expiresAt || 0) > Date.now()).sort((left, right) => Number(right.lastActiveAt || 0) - Number(left.lastActiveAt || 0)); list.replaceChildren(); if (!rooms.length) { const empty = document.createElement("span"); empty.className = "recent-empty"; empty.textContent = copy.recentEmpty; list.append(empty); return; } for (const room of rooms) { const button = document.createElement("button"); button.type = "button"; button.className = "recent-item" + (room.roomId === roomId ? " active" : ""); const name = document.createElement("span"); name.className = "recent-name"; name.textContent = copy.room + " · " + room.roomId.slice(-4).toUpperCase(); const meta = document.createElement("span"); meta.className = "recent-meta"; meta.textContent = formatRemaining(Number(room.expiresAt) - Date.now()) + " " + copy.expires; button.append(name, meta); button.addEventListener("click", () => { if (room.roomId === roomId) return; location.href = localePrefix + "/room/" + encodeURIComponent(room.roomId) + "?token=" + encodeURIComponent(room.relayToken) + "#" + room.encryptionKey; }); list.append(button); } }
    ...
    async function initialize() { const saved = await readRoom(roomId).catch(() => null); const url = new URL(location.href); state.relayToken = url.searchParams.get("token") || saved?.relayToken || ""; state.encryptionKey = location.hash.slice(1) || saved?.encryptionKey || ""; state.expiresAt = Number(saved?.expiresAt || Date.now() + 24 * 60 * 60 * 1000); state.authorId = saved?.authorId || randomId(16); state.nickname = saved?.nickname || localStorage.getItem("solomap-collaboration-nickname") || ""; nicknameInput.value = state.nickname; if (url.searchParams.has("token")) history.replaceState({}, "", url.pathname + location.hash); await renderRecentRooms(); if (!state.relayToken || !state.encryptionKey) { joinMessage.textContent = copy.missing; setStatus("error", copy.offline); } updateCountdown(); }
    joinButton.addEventListener("click", () => { state.nickname = nicknameInput.value.trim(); if (!state.nickname) { nicknameInput.focus(); return; } localStorage.setItem("solomap-collaboration-nickname", state.nickname); connect(); });
