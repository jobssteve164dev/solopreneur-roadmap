...
<head>
  ...
  <style>
    ...
    .workbench-room .collaboration-layout{display:block;width:100%;min-height:620px;margin:0;padding:0}.workbench-room .room-switcher{display:grid;grid-template-columns:auto minmax(220px,340px) minmax(0,1fr);align-items:center;gap:12px;margin:0 0 16px}.workbench-room .room-switcher label{font-size:13px;font-weight:750;color:var(--soft)}.workbench-room .room-switcher .input{min-height:42px;padding:8px 36px 8px 12px;background:var(--glass-bg)}.workbench-room .room-switcher .storage-note{margin:0;text-align:right}.workbench-room .room-shell{min-height:620px;border-radius:16px;box-shadow:none}@media(max-width:760px){.workbench-room .collaboration-layout{min-height:0}.workbench-room .room-switcher{grid-template-columns:1fr;gap:8px;margin-bottom:12px}.workbench-room .room-switcher .storage-note{text-align:left}.workbench-room .room-shell{min-height:620px}}
...
<body class="${workbench ? "workbench-room" : "standalone-room"}">
  ${workbench
    ? `${headerHtml}<div class="desk shell">${sidebarHtml}<main class="desk-main"><header class="desk-top"><div><h1>${escapeHtml(copy.workspaceTitle)}</h1><p>${escapeHtml(copy.workspaceLead)}</p></div></header>`
  ...
  <div class="collaboration-layout">
    ${workbench
      ? `<div class="room-switcher"><label for="recent-list">${escapeHtml(copy.recent)}</label><select class="input" id="recent-list" aria-label="${escapeHtml(copy.recent)}"></select><p class="storage-note">${escapeHtml(copy.storage)}</p></div>`
