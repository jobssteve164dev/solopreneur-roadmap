...
export function buildCollaborationRoomPage(roomId = "", locale = "en", options = {}) {
  ...
  const siteStyles = workbench ? String(options.siteStyles || "") : "";
  const workbenchStyles = workbench ? String(options.workbenchStyles || "") : "";
  const headerHtml = workbench ? String(options.headerHtml || "") : "";
  const sidebarHtml = workbench ? String(options.sidebarHtml || "") : "";
  const footerHtml = workbench ? String(options.footerHtml || "") : "";
  const primaryButtonClass = workbench ? "button primary" : "primary";
  const copy = zh ? {
    title: "临时共创 · SoloMap",
    eyebrow: "临时共创房间",
    back: "返回 SoloMap",
    ...
    workspaceTitle: "Co-create space",
    ...
    lobby: "Enter co-create lobby",
    joinAnother: "Join another room"
  };
...
<body class="${workbench ? "workbench-room" : "standalone-room"}">
  ...
  (() => {
    ...
    function switchRoom(room) { if (!room || room.roomId === roomId) return; if (state.socket && state.socket.readyState < WebSocket.CLOSING) state.socket.close(1000, "Switch room"); roomId = room.roomId; state.relayToken = room.relayToken; state.encryptionKey = room.encryptionKey; state.expiresAt = Number(room.expiresAt || 0); state.authorId = room.authorId || randomId(16); state.nickname = room.nickname || state.nickname; state.messages.clear(); nicknameInput.value = state.nickname; inviteField.hidden = true; connect(); }
    async function renderRecentRooms() { const list = document.getElementById("recent-list"); const rooms = (await readRooms()).filter(room => Number(room.expiresAt || 0) > Date.now()).sort((left, right) => Number(right.lastActiveAt || 0) - Number(left.lastActiveAt || 0)); list.replaceChildren(); if (list instanceof HTMLSelectElement) { const placeholder = document.createElement("option"); placeholder.value = ""; placeholder.textContent = rooms.length ? copy.recent : copy.recentEmpty; placeholder.disabled = true; placeholder.selected = !rooms.some(room => room.roomId === roomId); list.append(placeholder); for (const room of rooms) { const option = document.createElement("option"); option.value = room.roomId; option.textContent = copy.room + " · " + room.roomId.slice(-4).toUpperCase() + " · " + formatRemaining(Number(room.expiresAt) - Date.now()) + " " + copy.expires; option.selected = room.roomId === roomId; list.append(option); } list.disabled = !rooms.length; list.onchange = () => switchRoom(rooms.find(room => room.roomId === list.value)); return; } if (!rooms.length) { const empty = document.createElement("span"); empty.className = "recent-empty"; empty.textContent = copy.recentEmpty; list.append(empty); return; } for (const room of rooms) { const button = document.createElement("button"); button.type = "button"; button.className = "recent-item" + (room.roomId === roomId ? " active" : ""); const name = document.createElement("span"); name.className = "recent-name"; name.textContent = copy.room + " · " + room.roomId.slice(-4).toUpperCase(); const meta = document.createElement("span"); meta.className = "recent-meta"; meta.textContent = formatRemaining(Number(room.expiresAt) - Date.now()) + " " + copy.expires; button.append(name, meta); button.addEventListener("click", () => switchRoom(room)); list.append(button); } }
...
<body>
  ...
  <div class="desk shell">
    ...
    <main class="lobby-main">
      <header class="lobby-head"><div><h1>${escapeHtml(copy.title)}</h1><p>${escapeHtml(copy.lead)}</p></div><div class="lobby-head-actions"><div class="lobby-status" role="status" aria-live="polite"><span class="lobby-status-dot" id="status-dot"></span><span id="status-text">${escapeHtml(copy.connecting)}</span></div><a class="button secondary" href="${zh ? "/zh" : ""}/workbench/collaboration?view=rooms">${escapeHtml(copy.joinRoom)}</a></div></header>
...
export function collaborationRoomPageHeaders(options = {}) {
  ...
  return {
    ...
    "content-security-policy": [
      "default-src 'none'",
      workbench ? "style-src 'unsafe-inline' https://fonts.googleapis.com" : "style-src 'unsafe-inline'",
      "script-src 'unsafe-inline'",
      "connect-src 'self' ws: wss:",
      workbench ? "img-src 'self' https://raw.githubusercontent.com data:" : "img-src 'none'",
      ...(workbench ? ["font-src https://fonts.gstatic.com"] : []),
      "base-uri 'none'",
      workbench ? "form-action 'self'" : "form-action 'none'",
