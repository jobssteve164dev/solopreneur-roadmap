...
async function searchOpenVsx(query, pageSize, offset) {
  ...
  if (!response.ok) {
    throw new Error(`Open VSX search failed for "${query}": ${response.status} ${response.statusText}`);
  }
...
async function rankQuery(query, config) {
  const firstPage = await searchOpenVsx(query, config.pageSize, 0);
  ...
  for (let offset = 0; offset < scanLimit; offset += config.pageSize) {
    const page = offset === 0 ? firstPage : await searchOpenVsx(query, config.pageSize, offset);
...
function printTextReport(config, results) {
  ...
  for (const result of results) {
    ...
    console.log(`## ${result.query}: ${rankText}`);
    if (result.found) {
...
async function main() {
  ...
  for (const query of config.queries) {
    results.push(await rankQuery(query, config));
  }
