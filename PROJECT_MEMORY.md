...
- The sidebar exposes a Solo conversation card above the project portfolio. It lets the user choose a registered project and Agent CLI before sending, then activates that project only for dispatch so the run is stored in the selected project's existing `__solo__` history without opening its roadmap view first.
