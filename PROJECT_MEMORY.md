...
- Sidebar project card selection must stay fast: portfolio summaries are built from local roadmap files first, while GitHub Issue summaries are lazy-loaded after the card renders. The selected project's Issues load first; other project Issue counts can fill in afterward.
